...
/*void free_buddy(void *ptr) {
  ...
  Buddy* prev = buddyList[power].head;
  if(prev == to_be_freed) {
    buddyList[power].head = prev->next;
  } 
  else {
    while(prev->next != to_be_freed) {
      prev = prev->next;
    }
    prev->next = prev->next->next;
  }
  //buddyList[power].amt_avail--; //
  for(int i = power; i <= 20; i++) {
    void* buddyAddr;
    int currentSize = buddyList[i].size;
    int nextSize = currentSize * 2;
    unsigned long remaining = (ptr - space->start) % nextSize;
    if(remaining == 0) {
      buddyAddr = ptr + currentSize;
    }
    else if(remaining == currentSize) {
      buddyAddr = ptr - currentSize;
    }
    int intBuddy = (int)(buddyAddr - space->start);
    temp = buddyList[i].head;
    while(temp) {
      if(temp->address == buddyAddr) {
        break;
      }
      temp = temp->next;
    }
    if(temp == NULL || temp->free_or_allocated == 1) {
      to_be_freed->free_or_allocated = 0;
      to_be_freed->address = ptr;
      to_be_freed->next = buddyList[i].head;
      buddyList[i].head = to_be_freed;
      buddyList[i].amt_avail++;
      break;
    } 
    else {
      prev = buddyList[i].head;
      if(prev == temp) {
        buddyList[i].head = prev->next;
      } 
      else {
